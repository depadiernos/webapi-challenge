...
> 1. defining routes using different METHODS (GET, POST, PUT, DELETE)
> 2. using and creating middleware to extend the functionality of Express

- [x] Describe Middleware?
> Middleware intercepts a function, does something with the input, then passes it on. Literally a 'middle-man' for Express routes

- [x] Describe a Resource?
> A resource is a piece of data that is accessible through a specific API endpoint.

- [x] What can the API return to help clients know if a request was successful?
> The API endpoint can return an HTTP status code to inform the client of the current state of the request. `200 OK` is the status returned when a request succeeds.

- [x] How can we partition our application into sub-applications?
> We can 'componentize' our Express app using the `.Router()` express method. This allows us to define a sub-route in a separate file and then `require` and `use` the route in our main express file.
